...
def main():
    ...
    try:
        w_count = int(input("How many words you want to play with at the same time?: "))
    except:
        x = input("You should give integer!: ")
        while not x.isdigit():
            x = input("You should give integer!: ")
        w_count = int(x)
    ...
    while count > 0 and not_finished:
        print(f"You have {count} attempt(s)")
        ...
        for i in range(w_count):
            if is_done[i] == 0:
                ...
                print("Non-included letters: " + "".join(sorted(non_included_words[i])))
    ...
    if count == 0:
        print(f"You lost, game over. Your words was: {asked}")
    else:
